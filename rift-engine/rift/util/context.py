...
def resolve_inline_uris(user_response: str, server: 'Server') -> List[lsp.Document]:
    ...
    for match in matches:
        ...
        replacement = lookup_match(match, server)
        result.append(
            lsp.Document(f"uri://{match}", lsp.DocumentContext(replacement))
        )
    return result

def contextual_prompt(prompt: str, documents: List[lsp.Document], max_size: Optional[int] = None) -> str:
