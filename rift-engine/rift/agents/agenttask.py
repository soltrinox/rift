...
class AgentTask:
    ...
    async def run(self):
        ...
        self._running = True
        try:
            args = list(await self.args()) if callable(self.args) else list(self.args) if self.args else []
            kwargs = dict(await self.kwargs()) if callable(self.kwargs) else dict(self.kwargs) if self.kwargs else {}
