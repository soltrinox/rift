...
from rift.lsp import CreateFile, TextEdit, Range, TextDocumentEdit, TextDocumentIdentifier, WorkspaceEdit
import os

def apply_diff_edits(uri: TextDocumentIdentifier, old_content: str, new_content: str) -> WorkspaceEdit:
    ...
    for op, text in diff:
        ...
        end_char = char + last_line_chars if line_count == 0 else last_line_chars  # if we moved to a new line, start at char 0
        ...
        if op == -1:
            ...
            # text was added
            edits.append(TextEdit(Range.mk(line, char, line, char), text))  # new text starts at the current position
