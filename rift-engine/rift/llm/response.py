...
def replace_functions_in_document(ir_doc: IR, ir_blocks: IR, document: str) -> str:
    ...
    for function_declaration in function_declarations_in_document:
        function_in_blocks = ir_blocks.symbol_table.get(function_declaration.name)
        if isinstance(function_in_blocks, FunctionDeclaration):
            new_function_text = function_in_blocks.get_substring()
            modified_document = modified_document[:function_declaration.substring[0]] + new_function_text + modified_document[function_declaration.substring[1]:]
...
if __name__ == "__main__":
    ...
    new_document1 = replace_functions_from_code_blocks(code_blocks=code_blocks1, document=Test.document, language=language)
    print(f"new document1:\n{new_document1}\n")
    code_blocks2 = extract_blocks_from_response(Test.response2)
    new_document2 = replace_functions_from_code_blocks(code_blocks=code_blocks2, document=Test.document, language=language)
