<script lang='ts'>
  ...
  const MOCK_AGENT_IDS = ['rift-chat', 'gpt-engineer', 'auto-code-review', 'repl-auto-debug']
  ...
  function handleKeyDown(e: KeyboardEvent) {
    if(e.key === 'Enter') {
      ...
      state.update(state => ({...state, currentlySelectedAgentId: 'rift-chat'}))
    }
    if(e.key == 'ArrowDown') {
      e.preventDefault()
